...
First get rid of species that are not birds and birds from which we can calculate offsets (using QPAD). Then fix the individual count label as tmtt (i.e., too many to tag) in the ARU sensor using the wt_replace_tmtt function which considers the observer_id. 

```{r}
dat.tmtt <- dat %>%
  filter(species_code %in% spp)%>%                #filter species 
...
##4.3. Caclulate duration and distance
...
Also we fix the task_method so that all PC data points are no longer NA but "PC".
...
dat.wt <- dat.tmtt %>%  
  ...
  mutate(distance_replace= gsub("m","", survey_distance_method), #replace m
         ...
         duration=ifelse(is.na(duration_max), as.numeric(gsub("s","", task_duration)), duration_max),
...
##4.3. Calculate abundance and select columns
